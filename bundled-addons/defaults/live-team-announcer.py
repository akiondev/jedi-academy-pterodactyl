...
def announcement_player_name(name: str) -> str:
    """Return *name* with color codes intact but unsafe control characters removed."""
    return _CONTROL_CHAR_RE.sub("", name).strip()
...
def announcement_for(name: str, new_team: str) -> str | None:
    cleaned = announcement_player_name(name)
    if not cleaned:
        return None
    team = (new_team or "").strip().upper()
    if team == "RED":
        return f"{cleaned} joined ^1RED TEAM"
    if team == "BLUE":
        return f"{cleaned} joined ^4BLUE TEAM"
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    for raw in sys.stdin:
        ...
        rcon_command = f"{config['announce_command']} {quote_rcon_arg(message)}"
        try:
            send_rcon_command(
                config["rcon_host"],
                port,
