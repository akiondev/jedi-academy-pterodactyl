...
fail() {
  ...
  printf '%s\n' "- Confirm your own legally owned 32-bit (i386) Linux Jedi Academy MP dedicated server binary is uploaded to /mnt/server" >&2
...
prepare_selected_mod_directory() {
  ...
  warn "YBEProxy legacy32 does not auto-prepare any mod directories. Upload your mod folder into ${mod_path} before startup"
}
...
print_install_summary() {
  ...
  kv "Server binary  :" "$SERVER_BINARY"
...
: "${SERVER_BINARY:=./linuxjampded}"
: "${FS_GAME_MOD:=base}"

[[ "${COPYRIGHT_ACKNOWLEDGED}" == "true" ]] || fail "COPYRIGHT_ACKNOWLEDGED must be true. This egg does not distribute Jedi Academy assets or any dedicated server engine binary."

mkdir -p \
  /mnt/server/base \
  /mnt/server/logs \
  /mnt/server/addons \
  /mnt/server/addons/docs \
  /mnt/server/addons/examples \
  /mnt/server/addons/defaults \
  /tmp/ybeproxy-legacy32-install
...
if [[ ! -f "/mnt/server/base/${SERVER_CONFIG}" ]]; then
  cat > "/mnt/server/base/${SERVER_CONFIG}" <<CFG
seta sv_hostname "YBEProxy legacy32 Pterodactyl Server"
seta g_motd "Powered by YBEProxy legacy32 on Pterodactyl"
...
info "Engine binary is manual; YBEProxy game module is image-managed"
warn "This egg does not download, upload, fetch, build or unpack any Jedi Academy dedicated server engine binary"
warn "Provide your own legally owned 32-bit (i386) Linux Jedi Academy MP dedicated server binary at /mnt/server/${SERVER_BINARY#./} (e.g. linuxjampded, openjkded.i386 or taystjkded.i386)"
info "The image-managed YBEProxy game module (base/jampgamei386.so) will be staged into base/ on startup; do not place your own jampgamei386.so under base/ unless you intend to override it"
warn "Provide your own legally owned base assets manually in /mnt/server/base so that assets0.pk3 exists"

print_install_checks
print_install_inventory
print_path_status "Server binary" "/mnt/server/${SERVER_BINARY#./}"
info "Selected fs_game mod directory: ${active_game_dir}"
if is_base_mode "$active_game_dir"; then
  ok "Running against base assets only; no managed mod directory is created"
else
