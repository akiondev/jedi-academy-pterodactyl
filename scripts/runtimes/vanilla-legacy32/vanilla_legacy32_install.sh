...
fail() {
  ...
  printf '%s\n' "- Confirm your own legally owned vanilla 32-bit dedicated server binary is uploaded to /mnt/server" >&2
...
prepare_selected_mod_directory() {
  ...
  warn "Vanilla legacy32 does not auto-prepare any mod directories. Upload your mod folder into ${mod_path} before startup"
}
...
[[ "${COPYRIGHT_ACKNOWLEDGED}" == "true" ]] || fail "COPYRIGHT_ACKNOWLEDGED must be true. This egg does not distribute Jedi Academy assets or the vanilla dedicated server binary."
...
seta sv_hostname "Vanilla legacy32 Pterodactyl Server"
seta g_motd "Powered by vanilla legacy32 on Pterodactyl"
...
info "Game assets and engine binary are manual-only"
warn "This egg does not download, upload, fetch, build or unpack the vanilla Jedi Academy dedicated server binary"
warn "Provide your own legally owned 32-bit (i386) Linux vanilla dedicated server binary at /mnt/server/${SERVER_BINARY#./}"
