...
determine_runtime_ownership() {
  ...
  if is_taystjk_managed_mod_dir "$active_game_dir"; then
    if [[ "${TAYSTJK_AUTO_UPDATE_BINARY:-false}" == "true" ]]; then
      ...
      ACTIVE_MOD_OWNERSHIP="manual user-owned (TaystJK auto-update disabled)"
    fi
  ...
  if is_taystjk_managed_mod_dir "$active_game_dir"; then
    if [[ ! -d "$mod_path" ]]; then
      if [[ "${TAYSTJK_AUTO_UPDATE_BINARY:-false}" == "true" ]]; then
        ...
        fail "Configured manual TaystJK mod directory ${active_game_dir} was not found under /home/container. Set TAYSTJK_AUTO_UPDATE_BINARY=true to sync the bundled TaystJK payload from the image, or upload the mod folder yourself"
      fi
    fi
  elif is_base_mode "$active_game_dir"; then
...
print_paths() {
  ...
  if [[ "${ANTI_VPN_AKA_ENABLED:-true}" == "true" ]]; then
    kv "Anti-VPN AKA DB" "$ANTI_VPN_AKA_PATH"
  else
    kv "Anti-VPN AKA DB" "DISABLED"
  fi
