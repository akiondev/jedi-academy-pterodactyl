...
#     installed mode 0755 AND the bundled mod payload (taystjk/) is
#     mirrored under /home/container/taystjk/.
#   - With TAYSTJK_AUTO_UPDATE_BINARY=false neither the binary nor the
#     bundled mod payload is overwritten in the container volume; both
#     remain user-owned even when the active mod is `taystjk`. This is
#     the behavior that protects operator-replaced builds (e.g. a
#     custom jampgamex86_64.so or japro-assets.pk3).
...
assert_mode() {
  ...
  actual_mode="$(stat -c '%a' "$path" 2>/dev/null || stat -f '%Lp' "$path" 2>/dev/null || echo "")"
...
run_sync() {
  ...
  (
    ...
    export JKA_PATH_ENGINE_DIST JKA_PATH_ENGINE_BINARY_GLOB JKA_PATH_ENGINE_PAYLOAD_ROOT \
      JKA_CONTAINER_ROOT TAYSTJK_AUTO_UPDATE_BINARY
    sync_runtime_files
  )
}

# Case 1: auto-update on — both binary and bundled mod payload are
# installed from the image (image-managed runtime mode).
...
# Case 2: auto-update off — operator-supplied binary AND
# operator-replaced bundled-payload files (e.g. a custom
# jampgamex86_64.so or japro-assets.pk3) are both preserved.
...
mkdir -p "${case2}/home/container/taystjk"
printf 'operator-mod-file\n' > "${case2}/home/container/taystjk/cgame.qvm"
printf 'operator-jampgame\n' > "${case2}/home/container/taystjk/jampgamex86_64.so"
printf 'operator-japro-pk3\n' > "${case2}/home/container/taystjk/japro-assets.pk3"
run_sync "$case2" "false"
...
assert_file_contents "[auto-update-off] operator mod file preserved" \
  "${case2}/home/container/taystjk/cgame.qvm" \
  "operator-mod-file"
assert_file_contents "[auto-update-off] operator jampgamex86_64.so preserved" \
  "${case2}/home/container/taystjk/jampgamex86_64.so" \
  "operator-jampgame"
assert_file_contents "[auto-update-off] operator japro-assets.pk3 preserved" \
  "${case2}/home/container/taystjk/japro-assets.pk3" \
  "operator-japro-pk3"
