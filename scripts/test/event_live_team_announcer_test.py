...
class LiveTeamAnnouncerTest(unittest.TestCase):
    ...
    def test_team_change_event_triggers_rcon_announcement(self) -> None:
        ...
        try:
            ...
            self.assertIn("^1RED TEAM", decoded)
        finally:
            server.close()
    ...
    def test_spectator_and_free_team_produce_no_rcon_packet(self) -> None:
        for team in ("SPECTATOR", "SPECTATORS", "FREE"):
            with self.subTest(team=team):
                ...
                try:
                    ...
                    self.assertIsNone(
                        server.received,
                        msg=f"team={team!r} must not produce an RCON packet",
                    )
                finally:
                    server.close()

    def test_colored_name_with_slashes_is_preserved_and_quoted(self) -> None:
        server = _UDPServer()
        try:
            event = {
                "type": "team_change",
            ...
            self.assertIsNotNone(server.received, msg=f"no UDP packet received; stderr={stderr}")
            decoded = server.received.lstrip(b"\xff").decode("utf-8", errors="replace")
            # Color codes must be preserved, not stripped
            self.assertIn("^4^^0mYriad^4//^7kyoz^7", decoded)
            # The message must be wrapped in double quotes
            self.assertIn('svsay "^4^^0mYriad^4//^7kyoz^7 joined ^4BLUE TEAM"', decoded)
        finally:
            server.close()

    def test_red_and_blue_announcements_still_work(self) -> None:
            ...
            with self.subTest(team=team):
                ...
                try:
                    ...
                    for _ in range(20):
                        ...
                        time.sleep(0.05)
                    self.assertEqual(rc, 0, msg=stderr)
                    self.assertIsNotNone(server.received, msg=f"no UDP packet for team={team}; stderr={stderr}")
                    decoded = server.received.lstrip(b"\xff").decode("utf-8", errors="replace")
                    color = "^1" if team == "RED" else "^4"
                    self.assertIn(f"myr ^1. ^7 entry^7 joined {color}{team} TEAM", decoded)
                    # Quoted form
                    self.assertIn(f'svsay "myr ^1. ^7 entry^7 joined {color}{team} TEAM"', decoded)
                finally:
                    server.close()
