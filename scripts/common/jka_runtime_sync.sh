...
#     remain valid.
#   - the engine binary AND the bundled mod payload are gated together
#     behind TAYSTJK_AUTO_UPDATE_BINARY: when true, image-managed copies
#     overwrite anything already in /home/container; when false (the
#     default) operator-replaced builds (e.g. a custom
#     jampgamex86_64.so or japro-assets.pk3) are left untouched.
#
...
sync_runtime_files() {
  ...
  # TAYSTJK_AUTO_UPDATE_BINARY gates BOTH the engine binary
  # (taystjkded.x86_64) and the bundled mod payload (taystjk/, which
  # includes jampgamex86_64.so and japro-assets.pk3). When false (the
  # default) neither is overwritten so operator-replaced builds survive
  # restarts; when true the image-managed copies are synced on every
  # start.
  if [[ "$auto_update_binary" == "true" ]]; then
    if compgen -G "${JKA_PATH_ENGINE_DIST}/${JKA_PATH_ENGINE_BINARY_GLOB}" >/dev/null; then
      log "Image-managed auto-update enabled: syncing engine binaries from image into container volume"
    ...
    if [[ -d "${JKA_PATH_ENGINE_PAYLOAD_ROOT}" ]]; then
      log "Image-managed auto-update enabled: syncing bundled mod payload from image into container volume"
    ...
    log "TAYSTJK_AUTO_UPDATE_BINARY=false: leaving operator-managed binaries and bundled mod payload under ${container_root} untouched"
  fi
