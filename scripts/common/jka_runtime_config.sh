...
#     ONLY when the file is missing. Existing user-owned values are
#     preserved. No example file is
#     created next to it: the canonical schema/defaults are documented in
  ...
  cat <<'JSON'
{
  ...
  "anti_vpn": {
    ...
    "broadcast": {
      "mode": "pass-and-block",
      "cooldown": "90s",
      "pass_template": "svsay ^2VPN check passed for %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).",
      "block_template": "svsay ^1VPN or hosting network blocked: %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%)."
    },
    "aka": {
      "enabled": true,
      "path": "/home/container/.cache/taystjk-antivpn/aka.db",
      "max_entries": 20000,
      "ttl_days": 365
    },
    "enforcement": {
...
jka_runtime_config_migrate_missing_defaults() {
  return 0
}

# load_runtime_json_config ensures the config directory and example
# file exist, materialises the user file if it is missing, applies
# additive defaults migrations, validates the file as JSON, and exports
# the legacy environment variable names read by the rest of the runtime
# scripts.
#
...
load_runtime_json_config() {
  ...
  if ! jq -e . "$JKA_RUNTIME_CONFIG_PATH" >/dev/null 2>&1; then
    fail "Runtime config at ${JKA_RUNTIME_CONFIG_PATH} is not valid JSON after defaults migration"
  fi
  ...
  export FS_GAME_MOD SERVER_CONFIG SERVER_LOG_FILENAME SERVER_PORT
  ...
  ANTI_VPN_AKA_ENABLED="$(jka_runtime_config_query_or '.anti_vpn.aka.enabled' 'true')"
  ANTI_VPN_AKA_PATH="$(jka_runtime_config_query_or '.anti_vpn.aka.path' '/home/container/.cache/taystjk-antivpn/aka.db')"
  ANTI_VPN_AKA_MAX_ENTRIES="$(jka_runtime_config_query_or '.anti_vpn.aka.max_entries' '20000')"
  ANTI_VPN_AKA_TTL_DAYS="$(jka_runtime_config_query_or '.anti_vpn.aka.ttl_days' '365')"
  ANTI_VPN_AKA_TTL="$(jka_runtime_config_query_or '.anti_vpn.aka.ttl' "${ANTI_VPN_AKA_TTL_DAYS}d")"
  if [[ "$ANTI_VPN_AKA_TTL" =~ ^[0-9]+d$ ]]; then
    ANTI_VPN_AKA_TTL="$(( ${ANTI_VPN_AKA_TTL%d} * 24 ))h"
  fi
  ...
  ANTI_VPN_BROADCAST_PASS_TEMPLATE="$(jka_runtime_config_query_or '.anti_vpn.broadcast.pass_template' 'svsay ^2VPN check passed for %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).')"
  ANTI_VPN_BROADCAST_BLOCK_TEMPLATE="$(jka_runtime_config_query_or '.anti_vpn.broadcast.block_template' 'svsay ^1VPN or hosting network blocked: %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).')"
  ...
  export \
    ...
    ANTI_VPN_AKA_ENABLED ANTI_VPN_AKA_PATH ANTI_VPN_AKA_MAX_ENTRIES ANTI_VPN_AKA_TTL \
