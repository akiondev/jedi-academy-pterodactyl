...
configure_anti_vpn() {
  ...
  : "${ANTI_VPN_AKA_ENABLED:=true}"
  : "${ANTI_VPN_AKA_PATH:=/home/container/.cache/taystjk-antivpn/aka.db}"
  : "${ANTI_VPN_AKA_MAX_ENTRIES:=20000}"
  : "${ANTI_VPN_AKA_TTL:=8760h}"
  : "${ANTI_VPN_ENFORCEMENT_MODE:=kick-only}"
  : "${ANTI_VPN_BROADCAST_MODE:=pass-and-block}"
  : "${ANTI_VPN_BROADCAST_COOLDOWN:=90s}"
  : "${ANTI_VPN_BROADCAST_PASS_TEMPLATE:=svsay ^2VPN check passed for %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).}"
  : "${ANTI_VPN_BROADCAST_BLOCK_TEMPLATE:=svsay ^1VPN or hosting network blocked: %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).}"
  ...
  mkdir -p "$(dirname "$ANTI_VPN_AKA_PATH")"
}
...
print_anti_vpn_summary() {
  ...
  debug "AKA DB: $(printf '%s' "$(bool_state "${ANTI_VPN_AKA_ENABLED:-true}")" | tr '[:lower:]' '[:upper:]') ${ANTI_VPN_AKA_PATH}"
