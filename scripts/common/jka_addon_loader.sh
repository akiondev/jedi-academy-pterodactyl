...
#   ADDON_MATCH_ANNOUNCER_ENABLED           — true|false
#   ADDON_MATCH_ANNOUNCER_SCRIPT
#   ADDON_MATCH_ANNOUNCER_CONFIG_JSON
#
...
load_addons_json_config() {
  ...
  # shellcheck disable=SC2034  # consumed via eval below
  local section=""
  for name in announcer live_team_announcer chatlogger match_announcer; do
...
print_addon_summary() {
  ...
  kv "Match announcer" "$(addon_state_label "${ADDON_MATCH_ANNOUNCER_ENABLED:-false}")"
