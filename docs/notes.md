...
6. The Docker image should be built from the project root with `docker build -f docker/taystjk-modern64/Dockerfile ... .` so that `COPY scripts/entrypoint.sh` works reliably.
