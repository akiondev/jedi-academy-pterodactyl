...
The anti-VPN supervisor is configured through `/home/container/config/jka-runtime.json`. The runtime creates this file from the shipped template on first start and preserves existing user-owned values afterwards. New image revisions may add missing disabled default sections without resetting operator settings. The canonical schema is documented in `docs/addons/ADDON_README.md`. Edit the `anti_vpn`, `rcon_guard`, `addons`, and `supervisor` sections in that file to change behavior. **Do not put provider API keys into the Pterodactyl egg** — write them into the `anti_vpn.providers.*` keys in `jka-runtime.json` so they stay on disk in the operator's volume and are never logged to the console.
...
- public player notification path: server command templates. Default is `ANTI_VPN_BROADCAST_MODE=pass-and-block` and the shipped template uses `svsay` without exposing player IPs. Set the mode to `block-only` if you want allow decisions to be silent, or `off` to suppress all anti-VPN output.
...
- AKA path: a small local IP-to-nickname history at `/home/container/.cache/taystjk-antivpn/aka.db`. It is used only for server-console context (`AKA: ...`) and is never shown publicly in game.
...
`svsay ^2VPN check passed for %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).`

Default block template:

`svsay ^1VPN or hosting network blocked: %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).`

Real `ClientConnect` events always emit a fresh decision line, even when the same IP disconnects and reconnects inside the broadcast cooldown. The cooldown still applies to non-real duplicate events such as cached userinfo/reinit noise after map restart.

The supervisor also logs a console-only line for emitted decisions:

```text
VPN PASS: Player Country: Sweden AKA: PreviousName
```

If no provider supplied a country, it logs `Country: Unknown`. AKA history is IP-based, local to this server volume, capped by `ANTI_VPN_AKA_MAX_ENTRIES` (default `20000`) and pruned by `ANTI_VPN_AKA_TTL` (default `8760h`).
...
The image preserves existing values in `/home/container/config/jka-runtime.json`. Additive runtime migrations may insert missing disabled default sections, but they do not rewrite existing anti-VPN keys. As a result, servers that were originally provisioned on an older image keep their previous anti-VPN defaults — including the legacy `block-only` broadcast mode and `audit_allow=false`, both of which silently dropped allow decisions and made the system look broken.
