...
- TaystJK mod payload: image-managed under `/opt/jka/engine-payload/taystjk` (includes `jampgamex86_64.so` and `japro-assets.pk3`). It is mirrored into `/home/container/taystjk` only when the operator opts in by setting `TAYSTJK_AUTO_UPDATE_BINARY=true`; otherwise the contents of `/home/container/taystjk/` are treated as user-owned and left untouched, so operator-replaced builds survive restarts.
