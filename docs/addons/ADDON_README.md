...
>
> The runtime preserves existing JSON values once files exist on disk.
> Generic image revisions avoid adding gamecode-specific sidecars or
> service integrations. Custom server builds should package those in
> their own overlay image or top-level startup addon.
...
The image ships four default addons, all disabled by default:
...
| `match_announcer`     | `event`     | `match-announcer.py`       | Simple-only TFFA (gametype 6) match start/end announcements, armed by a passed `map_restart` callvote |
...
```json
{
  "addons": {
    "announcer": {
      ...
      "messages": [
        "Welcome to this Jedi Academy server. Have fun and play fair!"
      ]
    },
    "live_team_announcer": {
      "enabled": false,
...
`match_announcer`-specific keys:

| Key                      | Type   | Notes                                                                                      |
| ------------------------ | ------ | ------------------------------------------------------------------------------------------ |
| `announce_command`       | string | RCON command for all messages. Allowed: `svsay`, `say`. Default `svsay`. Used as fallback. |
| `match_start_command`    | string | RCON command for "Match Started" lines. Allowed: `svsay`, `say`. Falls back to `announce_command` if missing/invalid. |
| `match_end_command`      | string | RCON command for the single match-end line. Allowed: `svsay`, `say`. Falls back to `announce_command` if missing/invalid. |
| `gametype`               | int    | Gametype to track. Default and only supported value is `6` (TFFA). |
| `gametypes`              | array/string/int | Legacy compatibility key. If present, only `6` is accepted; all other values are ignored. |
| `require_map_restart_vote` | bool | When `true` (default), match tracking activates only after a passed `map_restart` callvote. |
| `messages`               | object | Message templates for the single start/end lines. Missing or non-string templates fall back to the built-in defaults. |

`match_announcer` is always simple-only:

* Match start emits exactly one RCON chat line.
* Match end emits exactly one RCON chat line.
* It emits no other messages between match start and match end.
* It does not emit late-join warnings.
* It does not emit roster, top-player, kill-count, or per-team summary lines.
* `simple_mode` from older configs is ignored; setting it to `false` does not restore multi-line output.
* `count_suicides` from older configs is ignored by the simple-only output.

Default message templates:

| Key | Default | Notes |
| --- | ------- | ----- |
| `tffa_start` | `^3Match Started ^7- ^2Good Luck, Have Fun!` | Start line for gametype 6. |
| `tffa_end_red` | `^3Match Ended ^7- ^1RED TEAM wins.` | TFFA RED win. May use `{red_score}` and `{blue_score}`. |
| `tffa_end_blue` | `^3Match Ended ^7- ^4BLUE TEAM wins.` | TFFA BLUE win. May use `{red_score}` and `{blue_score}`. |
| `tffa_end_draw` | `^3Match Ended ^7- ^7Draw.` | TFFA draw. May use `{red_score}` and `{blue_score}`. |

For TFFA, the winner is based on the final `red:X blue:Y` score line. RED > BLUE
uses `tffa_end_red`, BLUE > RED uses `tffa_end_blue`, and equal or missing scores
use `tffa_end_draw`.
