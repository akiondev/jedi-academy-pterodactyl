...
- `/home/container/config/jka-runtime.json` (default template; existing
  values are preserved; missing disabled default sections may be added
  by later images)
...
`server.log_filename`, `server.port_fallback` — is configured in
...
- `Configured manual TaystJK mod directory taystjk was not found
  ...
  `TAYSTJK_AUTO_UPDATE_BINARY=true` to sync the bundled TaystJK
  payload from the image, or upload the mod folder yourself.
