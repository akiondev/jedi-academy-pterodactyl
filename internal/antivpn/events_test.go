...
// TestParseTeamChangePreservesTrailingColorCode mirrors the exact TaystJK
// log line shape and verifies that the trailing ^7 reset code is preserved
// in the team_change event name so addons (e.g. live-team-announcer) can
// render the full colored player name.
func TestParseTeamChangePreservesTrailingColorCode(t *testing.T) {
	...
	want := teamChangeMatch{Slot: "0", IP: "90.144.88.223", Name: "akiondev^7", OldTeam: "BLUE", NewTeam: "RED"}
	if got != want {
		t.Fatalf("parseTeamChange mismatch: got %+v want %+v", got, want)
	}
}

// TestParseTeamChangePreservesColorCodedName verifies that a player name
// containing interior Quake/JKA color codes (e.g. ^4A^7kion) is published
// on the team_change event exactly as it appears in the ChangeTeam log line,
// so the live-team-announcer can announce "^4A^7kion joined RED TEAM".
func TestParseTeamChangePreservesColorCodedName(t *testing.T) {
	line := `ChangeTeam: 0 [90.144.88.223:29070] (GUID) "^4A^7kion" FREE -> RED`
	got, ok := parseTeamChange(line)
	if !ok {
		t.Fatalf("expected parseTeamChange to match line: %q", line)
	}
	want := teamChangeMatch{Slot: "0", IP: "90.144.88.223:29070", Name: "^4A^7kion", OldTeam: "FREE", NewTeam: "RED"}
	if got != want {
		t.Fatalf("parseTeamChange mismatch: got %+v want %+v", got, want)
	}
}
