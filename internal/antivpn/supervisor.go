...
const (
	antiVPNAnsiReset  = "\x1b[0m"
	antiVPNAnsiGreen  = "\x1b[32m"
	antiVPNAnsiRed    = "\x1b[31m"
	antiVPNAnsiYellow = "\x1b[33m"
	antiVPNAnsiPurple = "\x1b[35m"
)
...
func NewSupervisor(cfg Config, logger *slog.Logger) (*Supervisor, error) {
	...
	return &Supervisor{
		...
		engine:          engine,
		akaStore:        newAkaStore(cfg, logger),
...
func (s *Supervisor) clearConnectionState(slot string) {
	...
	// The broadcast cooldown is intentionally NOT cleared here. Real ClientConnect
	// broadcasts bypass the cooldown, but non-real duplicates (userinfo/cache
	// replays) still use the IP-level window below.
...
func (s *Supervisor) broadcastDecision(stdin io.Writer, slot, playerName string, decision Decision, triggerKind string) {
	...
	if !s.shouldBroadcast(decision, triggerKind) {
		return
	}
...
func (s *Supervisor) logVpnStatus(playerName string, decision Decision, country string) {
	aka := "-"
	if s.akaStore != nil {
		aka = s.akaStore.Record(decision.IP, playerName, country, time.Now().UTC())
	}
	...
	s.writeVpnStatusConsoleLine(status, playerName, country, aka)
...
func (s *Supervisor) writeVpnStatusConsoleLine(status string, playerName string, country string, aka string) {
	statusColor := antiVPNAnsiGreen
	if status == "BLOCK" {
		statusColor = antiVPNAnsiRed
	}

	fmt.Fprintf(
		os.Stdout,
		"VPN %s%s%s: %s Country: %s%s%s AKA: %s%s%s\n",
		statusColor,
		status,
		antiVPNAnsiReset,
		sanitizePlayerName(playerName),
		antiVPNAnsiYellow,
		sanitizeCountryForConsoleCommand(country),
		antiVPNAnsiReset,
		antiVPNAnsiPurple,
		sanitizePlayerName(aka),
		antiVPNAnsiReset,
	)
}

func (s *Supervisor) shouldBroadcast(decision Decision, triggerKind string) bool {
	...
	if triggerKind == connectKindReal {
		return true
	}

	now := time.Now().UTC()
	// Key non-real broadcasts by IP only (no slot) so duplicate userinfo/cache
	// events are still cooled down. Genuine ClientConnect events bypass this
	// window; a real disconnect/reconnect should always get a fresh native line.
...
func sanitizeCountryForConsoleCommand(value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
		return "Unknown"
	}
	...
	if len([]rune(safe)) > 32 {
		runes := []rune(safe)
		safe = string(runes[:32])
	}
	return safe
}

func decisionCountry(decision Decision) string {
	for _, provider := range decision.Providers {
		if !provider.Success {
			continue
		}
		country := firstNonEmpty(provider.CountryName, provider.CountryCode)
		if country != "" {
			return sanitizeCountryForConsoleCommand(country)
		}
	}
	return "Unknown"
}
