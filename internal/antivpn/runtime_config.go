...
func applyJSONOverrides(cfg Config, jsonCfg *jsonRuntimeConfig) (Config, error) {
	...
	if jsonCfg.AntiVPN.Aka.Enabled != nil {
		cfg.AkaEnabled = *jsonCfg.AntiVPN.Aka.Enabled
	}
	if v := strings.TrimSpace(jsonCfg.AntiVPN.Aka.Path); v != "" {
		cfg.AkaPath = v
	}
	if jsonCfg.AntiVPN.Aka.MaxEntries != nil && *jsonCfg.AntiVPN.Aka.MaxEntries >= 0 {
		cfg.AkaMaxEntries = *jsonCfg.AntiVPN.Aka.MaxEntries
	}
	if jsonCfg.AntiVPN.Aka.TTLDays != nil && *jsonCfg.AntiVPN.Aka.TTLDays >= 0 {
		cfg.AkaTTL = time.Duration(*jsonCfg.AntiVPN.Aka.TTLDays) * 24 * time.Hour
	}
	if v := strings.TrimSpace(jsonCfg.AntiVPN.Aka.TTL); v != "" {
		if d, err := parseDurationOrSeconds(v); err == nil {
			cfg.AkaTTL = d
		}
	}
