...
func TestFillCommandTemplateSupportsBroadcastPlaceholders(t *testing.T) {
	command := fillCommandTemplate(
		"svsay VPN check passed for %PLAYER% (%COUNTRY%, %SCORE%/%THRESHOLD%).",
		commandTemplateData{
			Player:    "Player",
			Score:     10,
			Threshold: 90,
	...
	expected := "svsay VPN check passed for Player (Sweden, 10/90)."
	if command != expected {
...
func TestClearConnectionStateClearsSeenEvents(t *testing.T) {
	...
	// Broadcast cooldown entries survive disconnect for non-real duplicate
	// events. Genuine ClientConnect broadcasts bypass this cooldown elsewhere.
...
func TestRealConnectBypassesBroadcastCooldownForRapidReconnect(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	supervisor := &Supervisor{
		cfg: Config{
			BroadcastMode:        BroadcastPassAndBlock,
	...
	supervisor.broadcastDecision(&stdin, "0", "Padawan", decision, connectKindReal)
	if stdin.Len() == 0 {
		t.Fatal("expected first broadcast to be sent")
	}

	// Real disconnect/reconnect must get a new native anti-VPN row even inside
	// the cooldown window.
	stdin.Reset()
	supervisor.broadcastDecision(&stdin, "0", "Aki", decision, connectKindReal)
	if stdin.Len() == 0 {
		t.Fatal("expected rapid reconnect broadcast to be sent")
	}

	stdin.Reset()
	supervisor.broadcastDecision(&stdin, "1", "ASF", decision, connectKindReal)
	if stdin.Len() == 0 {
		t.Fatal("expected rapid reconnect on another slot to be sent")
	}
}

func TestBroadcastCooldownStillSuppressesNonRealDuplicates(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	supervisor := &Supervisor{
		cfg: Config{
			BroadcastMode:        BroadcastPassAndBlock,
	...
	var stdin bytes.Buffer
	supervisor.broadcastDecision(&stdin, "0", "Player", decision, triggerKindUserinfo)
	if stdin.Len() == 0 {
		t.Fatal("expected first non-real broadcast to be sent")
	}

	stdin.Reset()
	supervisor.broadcastDecision(&stdin, "0", "Player", decision, triggerKindUserinfo)
	if stdin.Len() != 0 {
		t.Fatalf("expected duplicate non-real broadcast within cooldown to be suppressed, got: %q", stdin.String())
	}
}
