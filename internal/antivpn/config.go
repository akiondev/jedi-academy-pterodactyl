...
func LoadConfigFromEnv() (Config, error) {
	cfg := Config{
		...
		AkaEnabled:               envBool("ANTI_VPN_AKA_ENABLED", true),
		AkaPath:                  envString("ANTI_VPN_AKA_PATH", "/home/container/.cache/taystjk-antivpn/aka.db"),
		AkaMaxEntries:            envInt("ANTI_VPN_AKA_MAX_ENTRIES", 20000),
		AkaTTL:                   envDuration("ANTI_VPN_AKA_TTL", 365*24*time.Hour),
		BanCommand:               envString("ANTI_VPN_BAN_COMMAND", ""),
		KickCommand:              envString("ANTI_VPN_KICK_COMMAND", "clientkick %SLOT%"),
		BroadcastPassCommand:     envString("ANTI_VPN_BROADCAST_PASS_TEMPLATE", `svsay ^2VPN check passed for %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).`),
		BroadcastBlockCommand:    envString("ANTI_VPN_BROADCAST_BLOCK_TEMPLATE", `svsay ^1VPN or hosting network blocked: %PLAYER% ^7(%COUNTRY%, %SCORE%/%THRESHOLD%).`),
	...
	if cfg.AkaMaxEntries < 0 {
		return Config{}, fmt.Errorf("ANTI_VPN_AKA_MAX_ENTRIES must be zero or greater")
	}
	if cfg.AkaTTL < 0 {
		return Config{}, fmt.Errorf("ANTI_VPN_AKA_TTL must be zero or greater")
	}
	...
	if strings.TrimSpace(cfg.AkaPath) != "" {
		cfg.AkaPath = filepath.Clean(cfg.AkaPath)
	}
