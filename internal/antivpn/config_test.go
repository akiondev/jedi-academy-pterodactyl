...
func TestLoadConfigDefaultsToAkaStore(t *testing.T) {
	t.Setenv("JKA_RUNTIME_CONFIG_PATH", "/nonexistent/jka-runtime.json")
	...
	if !cfg.AkaEnabled {
		t.Fatal("expected AKA store to be enabled by default")
	}
	if cfg.AkaPath != "/home/container/.cache/taystjk-antivpn/aka.db" {
		t.Fatalf("unexpected AKA path: %q", cfg.AkaPath)
	}
	if cfg.AkaMaxEntries != 20000 {
		t.Fatalf("expected AKA max entries 20000, got %d", cfg.AkaMaxEntries)
	}
}
