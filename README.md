...
> **Sibling runtime:** This repository also ships an isolated **Jedi Outcast / JK2MV** dedicated server runtime under
> [`docker/jk2mv/`](docker/jk2mv/Dockerfile), [`scripts/jk2mv/`](scripts/jk2mv/), [`image/jk2mv/`](image/jk2mv/runtime.json) and
> [`egg/egg-jk2mv-pterodactyl.json`](egg/egg-jk2mv-pterodactyl.json), published as
> `ghcr.io/akiondev/jedi-academy-pterodactyl:jk2mv`. It is fully separate from the TaystJK runtime described below — see
> [`docs/jk2mv.md`](docs/jk2mv.md) for details.
...
| `TAYSTJK_AUTO_UPDATE_BINARY` | When `true`, the runtime overwrites both `/home/container/taystjkded.x86_64` (engine binary) and the bundled `/home/container/taystjk/` mod payload (including `jampgamex86_64.so` and `japro-assets.pk3`) from the image-managed TaystJK build on every start, ignoring `SERVER_BINARY`. When `false` (default), no engine binary and no bundled mod files are ever overwritten — operator-replaced builds survive restarts. |
...
The runtime preserves existing `jka-runtime.json` values once the file exists. Generic image revisions avoid adding gamecode-specific sidecars or service integrations; custom server builds should package those in their own private overlay image or startup addon.
...
- when `TAYSTJK_AUTO_UPDATE_BINARY=true`, the image-managed `taystjkded.x86_64` AND the bundled `taystjk/` mod payload (including `jampgamex86_64.so` and `japro-assets.pk3`) are synced into `/home/container` on every start
- when `TAYSTJK_AUTO_UPDATE_BINARY=false` (default), neither the engine binary nor the bundled mod payload is overwritten — any operator-replaced builds under `/home/container/taystjk/` are preserved across restarts
...
- with `TAYSTJK_AUTO_UPDATE_BINARY=false` (the default) the bundled `taystjk/` directory is also user-owned, so operator-replaced game modules and asset PK3s survive restarts
...
- `taystjkded.*` and the bundled `taystjk/` mod payload are image-managed only when `TAYSTJK_AUTO_UPDATE_BINARY=true`; with the default `false` both are user-owned namespaces
...
- Runtime behavior: captures join events from live server stdout/stderr, caches decisions locally, writes a dedicated audit trail (allow rows included by default via `anti_vpn.audit_allow=true`), stores console-only IP-based AKA context locally, can kick blocked players with `clientkick`, and can emit public `svsay` PASS/BLOCK messages when `anti_vpn.broadcast.mode = "pass-and-block"`
